...
functional_groups = [Chem.MolFromSmarts(smi) for smi in list(functional_groups_smarts.values())]
...
def generate_y(mols, logic):
  ...
  if logic == 'logic_4':
    ...
    Y = [1 if len(mol.GetSubstructMatch(patt1))>0 or len(mol.GetSubstructMatch(patt2))<1 
           else 0 for mol in mols]
  elif logic == 'logic_10':
    patt1 = Chem.MolFromSmarts("[NX3;H2;!$(NC=[!#6]);!$(NC#[!#6])][#6]") # Primary amine not amide
    patt2 = Chem.MolFromSmarts("[OD2](C)C") # Ether
    patt3 = Chem.MolFromSmarts("[cR1]1[cR1][cR1][cR1][cR1][cR1]1") # Benzene
    Y = [1 if (len(mol.GetSubstructMatch(patt1))>0 and len(mol.GetSubstructMatch(patt2))<1) 
           or (len(mol.GetSubstructMatch(patt2))<1 and len(mol.GetSubstructMatch(patt3))<1)
           else 0 for mol in mols]
  elif logic == 'logic_13': 
    ...
    Y = [1 if (len(mol.GetSubstructMatch(patt1))>0 and len(mol.GetSubstructMatch(patt2))<1) 
           or (len(mol.GetSubstructMatch(patt3))>0 and len(mol.GetSubstructMatch(patt4))<1)
           else 0 for mol in mols]
...
def make_chem_data(logic_id):
  ...
  try:
    X_train, X_test, Y_train, Y_test = prepare_chem_dataset("{}/logic_{}_train.csv".format(basepath, logic_id), 
    ...
    print('Data not found, please download at https://github.com/google-research/graph-attribution/raw/main/data/all_16_logics_train_and_test.zip and place in datasets/chem_data')
    exit()
