...
class FeatureSelector(nn.Module):
    ...
    def __init__(self, input_dim, sigma):
        ...
        self.mu = torch.nn.Parameter(0.1*torch.randn(input_dim, ), requires_grad=True)
...
class SingleSTG(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, sigma):
        ...
        self.to_latent = nn.Sequential(nn.Linear(input_dim, hidden_dim),
...
class EnsembleSTG(nn.Module):
    ...
    def __init__(self, config_dict):
        ...
        self.stgs = nn.ModuleList([SingleSTG(config_dict['in_dim'], config_dict['h_dim'], config_dict['sigma']) for i in range(self.num_stg)])
